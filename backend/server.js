...
if (process.env.VAPID_PUBLIC_KEY && process.env.VAPID_PRIVATE_KEY) {
  try {
    ...
    console.warn(
      "VAPID keys configuration failed (push notifications disabled):",
      error.message,
    );
  }
} else {
...
const corsOptions = {
  ...
  exposedHeaders: ["X-Request-ID", "X-Server-ID"],
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    if (origin && origin !== process.env.FRONTEND_URL) {
      ...
      return res.status(403).json({
        ...
        message:
          "This API only accepts requests from authorized frontend applications",
      });
    }
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (isSuspicious) {
      ...
      return res.status(403).json({
        ...
        message:
          "External API tools are blocked. Use the frontend application.",
      });
    }
  }
...
app.get(
  "/api/system/maintenance/status",
  require("./controllers/systemMaintenanceController").getMaintenanceStatus,
);
...
app.get("/api/patch-notes/public", async (_, res) => {
  try {
...
app.use(
  ...
  protect,
  require("./routes/maintenanceAnnouncementRoutes"),
);
