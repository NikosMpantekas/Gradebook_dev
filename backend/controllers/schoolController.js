...
const createSchool = asyncHandler(async (req, res) => {
  ...
  const sanitizedSchoolDomain =
    schoolDomain || name.toLowerCase().replace(/\s+/g, "");
...
const getSchools = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (req.user.role === "superadmin") {
      ...
      console.log(
        `Found ${schools.length} schools for admin domain: ${adminDomain}`,
      );
    } else if (
      req.user.role === "secretary" &&
      req.user.secretaryPermissions?.canManageSchools
    ) {
      ...
      console.log(
        `Filtering schools for secretary with domain: ${secretaryDomain}`,
      );
