...
const validateEmail = (email) => {
  ...
  return (
    emailRegex.test(email) && email.length <= SECURITY_CONFIG.MAX_EMAIL_LENGTH
  );
};
...
const Contact = () => {
  ...
  const handleInputChange = (e) => {
    ...
    const allowSpaces =
      name === "message" || name === "name" || name === "subject";
    const sanitizedValue = sanitizeInput(
      value,
      SECURITY_CONFIG[`MAX_${name.toUpperCase()}_LENGTH`],
      allowSpaces,
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let errorMessage =
        "Προέκυψε σφάλμα κατά την αποστολή του μηνύματος. Παρακαλώ δοκιμάστε ξανά.";

      if (error.response) {
        if (error.response.status === 429) {
          errorMessage =
            "Πάρα πολλές προσπάθειες. Παρακαλώ περιμένετε λίγο πριν δοκιμάσετε ξανά.";
          ...
          errorMessage =
            "Πρόβλημα με τον διακομιστή. Παρακαλώ δοκιμάστε ξανά αργότερα.";
        }
      } else if (error.code === "ECONNABORTED") {
        errorMessage =
          "Η σύνδεση έληξε. Παρακαλώ ελέγξτε τη σύνδεσή σας και δοκιμάστε ξανά.";
